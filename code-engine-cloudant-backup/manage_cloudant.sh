...
# Modified for IBM Code Engine - .env file is optional
...
# Load environment variables from .env file if it exists (optional for Code Engine)
if [ -f .env ]; then
    source .env
    echo "Loaded environment variables from .env file"
else
    echo "No .env file found - using environment variables from Code Engine"
fi

# --- CONFIGURATION ---
# Environment variables can be set via:
# 1. Code Engine Secrets (for sensitive data like COUCH_URL)
# 2. Code Engine ConfigMaps (for non-sensitive config)
# 3. Direct environment variables in job definition
# 4. Local .env file (for testing)
...
Optimized for IBM Code Engine execution.
...
For Code Engine:
  - Set sensitive variables (COUCH_URL, AWS credentials) as Secrets
  - Set non-sensitive config as ConfigMaps or environment variables
  - No .env file needed when running in Code Engine
EOF
}

main() {
    # Default to 'backup' if no command is provided.
